...
useEffect(() => {
  ...
  const modeTimeRef1 = child(dbRef, `Node1/Zone2/ModeTime/1${username}`);
  const unsubscribe1 = onValue(modeTimeRef1, (snapshot) => {
    if (snapshot.exists()) {
      const data = snapshot.val();
      seton1(data["Time12_1"]);
      setoff1(data["Time12_2"]);
      setday1(data["Day12_1"]);
    } else {
      console.log("No data available");
    }
  }, (error) => {
...
const modeTimeRef2 = child(dbRef, `Node1/Zone2/ModeTime/2${username}`);
  const unsubscribe2 = onValue(modeTimeRef2, (snapshot) => {
    if (snapshot.exists()) {
      const data = snapshot.val();
      seton2(data["Time12_3"]);
      setoff2(data["Time12_4"]);
      setday2(data["Day12_2"]);
      setStatus2(data["Status12_2"]);
    } else {
      console.log("No data available");
    }
  }, (error) => {
  ...
  const [Time12_1, setTime1] = useState('');
  const [Time12_2, setTime2] = useState('');
  const [Day12_1, selectedDay11_1] = useState('');
  

  function updateDatatime1() {
    const path = `Node1/Zone2/ModeTime/1${String(username)}`;
  ...
  const [Time12_3, setTime3] = useState('');
  const [Time12_4, setTime4] = useState('');
  const [Day12_2, selectedDay11_2] = useState('');

  function updateDatatime2() {
    const path = `Node1/Zone2/ModeTime/2${String(username)}`;
    const db = getDatabase();
    const dbRef = ref(db, path);
    update(dbRef, {
      Time12_3: Time12_3,
